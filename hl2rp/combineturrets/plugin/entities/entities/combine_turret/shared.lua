...
function Pl:IsCombineFaction(faction)
	return (faction == FACTION_MPF or faction == FACTION_OTA);
end;

function Pl:PlayerIsCombine(player, bHuman)
...
function ENT:IsValidTarget(target)
	return target:IsPlayer() and !Schema:PlayerIsCombine(target) and target:Alive() and self:GetForward():Dot(self:GetAnglesToTarget(target):Forward()) >= self.MinimumAimDot and TrueVisible(self:ShootPos(), self:GetTargetPos(target), self)
end
...
function ENT:GetScanFilter()
	local filter = team.GetPlayers(CLASS_EMP)
...
function ENT:CanBePackedBy(pl)
	local owner = self:GetObjectOwner()
	return not owner:IsValid() or owner == pl or owner:Team() ~= CLASS_EMP or gamemode.Call("PlayerIsAdmin", pl)
end
