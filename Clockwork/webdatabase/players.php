...
die('<div class="alert alert-error"><b>Error</b><br>You need to login</div>');
       } elseif (isset($_SESSION['steamlogin'])) {
        $usersteamid = $steam_login_verify;
        $convertedsteamid = steamid64convert($usersteamid);
         if (issuperadmin($convertedsteamid) == "1") {
          
         }
        else die('<div class="alert alert-error"><b>Error</b><br>You are not a Superadmin</div>');
       }
...
$result = mysql_query("SELECT *  FROM `players` WHERE `_Schema` = '".$gamemodecode."' ORDER BY _SteamName LIMIT $offset, $rec_limit");
if (mysql_error()==""){ 
