...
GithubIssuesWidget.go = function () {
  $('#github-issues-widget').append('<span class="label label-warning">Loading</span>');
  $.getJSON(this.url, function (data) {
    var list = $('<table class="table table-striped"><tr><th>Issue</th><th>Status</th></tr></table>');
    $.each(data.data, function (issueIndex, issue) {
      var issueHtml = '<tr style="' + style + '"><td>';
      ...
      $.each(issue.labels, function (labelIndex, label) {
        ...
        issueHtml += '<td><span class="label" style="' + style + 'color: #000000;">' + label.name + '</span></td>';
      });
      issueHtml += "</tr>";
      list.append(issueHtml);
    });
    $('#github-issues-widget span.label-warning').remove();
